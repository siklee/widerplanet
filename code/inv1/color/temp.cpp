...
cvtColor(src, src_gray, CV_BGR2GRAY);                        //rgb를 그레이 영상으로
...
/*
Mat ycrcb_detect_pointer(Mat& src)                                           //ycrcb 컬러영역을 이용한 구분
{
	Mat img;
	Mat skin;
	int row, col;
	...
	cvtColor(img, img, CV_BGR2YCrCb);                               //ycrcb 변환
...
//1.히스토그램 분포(파일)
		case 1:
			init();                                         //히스토그램을 만들기 위한 동적할당
			while (1)
			{
				sprintf(imagename, "C:/Users/user/Desktop/숙제/capstone/자료/input/%d.jpg", count_img);
...
void show_hist(const char *channel, int range, int chnum)
{
	...
	sprintf(filename, "C:/Users/user/Desktop/숙제/capstone/자료/%s.txt", channel);
	ofstream histout;
	...
	namedWindow(channel);
	imshow(channel, temp);
	sprintf(filename, "C:/Users/user/Desktop/숙제/capstone/자료/%s.jpg", channel);
...
/*
Mat kovac(Mat& src)                                                //RGB 컬러영역을 이용한 구분 
{
	int i, j;
	Mat rgb;
...
Mat hls_detect(Mat& src)                                     //hls 컬러영역을 이용한 구분 
{
	...
	cvtColor(skinImg, hlsImg, CV_BGR2HLS);                             //hls로 변환 
	vector<Mat> hls_images(3);
	split(hlsImg, hls_images);                                         // 채널별로 분할
...
Mat ycrcb_detect(Mat& src)                                           //ycrcb 컬러영역을 이용한 구분
{
	Mat img;
	Mat skin;
	int i, j;
...
/*
Mat merge_detect(Mat& a, Mat& b)                           //각각 구분을 한 영상을 합치는 함수 진행중
{
	Mat img;
	a.copyTo(img);
...
//image = imread("C:/Users/user/Desktop/숙제/capstone/자료/testcase/155.jpg", CV_LOAD_IMAGE_COLOR);   // Read the file(jpg)
