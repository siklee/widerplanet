...
void pro_detect(const char *img_name, Mat& src, ofstream& ofile,ofstream& ufile,ofstream& sfile,int th);              //skin ÃßÃâÇÔ¼ö 
Mat cropimg(Mat& src);
...
bool name_detect(char *li,unordered_map<string, int> word);
bool name_detect2(char *li,unordered_map<string, int> word2);

int count_img = 0;                           // ÀüÃ¼ ÀÌ¹ÌÁö °³¼ö
float count_cor = 0;                         // °É·¯¾ß µÇ´Â ÀÌ¹ÌÁö
float count_fal = 0;                         // °Å¸£Áö ¸»¾Æ¾ßÇÒ ÀÌ¹ÌÁö 
...
cv::Mat curlImg(const char *img_url, int timeout = 10)
{
	...
	cout << img_url << endl;
	image = imdecode(stream, -1); // 'keep-as-is'
	return image;
}

cv::Mat curl2Img(const char *img_url, const char *path)
{
	CURL* curl;
    FILE* fp;
	CURLcode res;
